...
describe("deleteAvatarInput", () => {
  it("should delete avatar when user is authenticated", async () => {
    ...
    const deleteResponse = await createClient({
      token,
    }).deleteAvatarInput.mutate(deleteInput)

    ok(deleteResponse)
    partialDeepStrictEqual(deleteResponse, {
      ...avatarInput,
      id: createResponse.id,
      userId: user.id,
    })
  ...
  it("should throw error when user is not authenticated", async () => {
    ...
    await rejects(
      createClient().deleteAvatarInput.mutate(deleteInput),
      partialRight(
        isMatchTRPCError,
  ...
  it("should throw error when avatar input is not found", async () => {
    ...
    await rejects(
      createClient({ token }).deleteAvatarInput.mutate(
        deleteInput
      ),
      partialRight(
        isMatchTRPCError,
  ...
  it("should throw error when trying to delete avatar input with wrong userId", async () => {
    ...
    await rejects(
      ...
      "Should return avatar input not found error when trying to delete with wrong userId"
    )
  })
})